<?php
// Ambil data hanya yang statusnya 'selesai'
...
<div class="no-print mb-10 flex justify-between">
  ...
  <button onclick="window.print()" class="bg-orange-500 text-white px-6 py-2 rounded shadow">Klik untuk Cetak / Save PDF</button>
</div>
...
<table class="w-full border-collapse border border-gray-300">
  ...
  <tbody>
    ...
    ?>
    <tr>
      <td class="border border-gray-300 p-2 text-sm"><?= date('d/m/Y', strtotime($row['created_at'])) ?></td>
      <td class="border border-gray-300 p-2 text-sm"><?= htmlspecialchars($row['nama_pembeli']) ?></td>
      <td class="border border-gray-300 p-2 text-sm"><?= $row['nama_produk'] ?></td>
      <td class="border border-gray-300 p-2 text-sm text-right"><?= $row['stok'] ?></td>
      <td class="border border-gray-300 p-2 text-sm text-right">Rp <?= number_format($subtotal, 0, ',', '.') ?></td>
    </tr>
    <?php endwhile; ?>
  </tbody>
  <tfoot>
    <tr class="bg-gray-200 font-bold">
      ...
      <td class="border border-gray-300 p-2 text-right text-orange-600">Rp <?= number_format($total_omzet, 0, ',', '.') ?></td>
    </tr>
  </tfoot>
</table>
