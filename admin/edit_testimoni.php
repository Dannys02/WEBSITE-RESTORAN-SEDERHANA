...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-md border border-slate-200">
  <div class="flex justify-between items-center mb-6">
    ...
    <a href="index.php?page=testimoni" class="text-gray-500 hover:text-blue-500 transition">Batal</a>
  </div>
  ...
  <form action="" method="POST" class="space-y-4">
    ...
    <div>
      ...
      <input type="text" name="nama_pelanggan" value="<?= htmlspecialchars($t['nama_pelanggan']) ?>"
      class="w-full border border-slate-200 p-3 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none transition" required>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="block text-sm font-bold mb-1">Pekerjaan</label>
        <input type="text" name="pekerjaan" value="<?= htmlspecialchars($t['pekerjaan']) ?>"
        class="w-full border border-slate-200 p-3 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none transition" required>
      </div>
      <div>
        <label class="block text-sm font-bold mb-1">Rating Bintang</label>
        <select name="bintang" class="w-full border border-slate-200 p-3 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none transition">
          <?php for ($i = 5; $i >= 1; $i--): ?>
    ...
    <div>
      ...
      <textarea name="isi" rows="5"
        class="w-full border border-slate-200 p-3 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none transition" required><?= htmlspecialchars($t['isi']) ?></textarea>
    </div>

    <div class="pt-4">
      <button type="submit" name="update_testimoni_ini"
        class="w-full bg-blue-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-600 active:scale-95 transition-all">
        Simpan Update
      </button>
    </div>
  </form>
